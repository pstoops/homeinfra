...
function print_help(){
  ...
  echo "Examples:"
  echo "${0} -t A -u add -n ns1 -v 192.168.1.246"
  echo "${0} -t A -u del -n ns1 -v 192.168.1.246"
  echo "${0} -t CNAME -u add -n ns3 -v ns1.home.taurus"
  echo "${0} -t CNAME -u del -n ns3 -v ns1.home.taurus"
  echo "${0} -t PTR -u add -n 192.168.1.246 -v ns1.home.taurus"
  echo "${0} -t PTR -u del -n 192.168.1.246 -v ns1.home.taurus"
  exit 1
}
...
function update_record(){
  ...
  if [[ $action == "add" ]]; then
    ...
    if [[ $? -eq 0 ]]; then
      ...
      sed -i /"}"/d ${basedir}/${filename}.yml
      sed -i -e 's/"$/",/' ${basedir}/${filename}.yml
      echo "  $servername: \"$record_value\"" >> ${basedir}/${filename}.yml
      echo "}"  >> ${basedir}/${filename}.yml
    fi
  fi
